...
const ProductPage: React.FC<ProductPageProps> = ({ product, onOrderNow, onNavigate }) => {
  ...
  return (
    <div className="bg-white min-h-screen relative">
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-[250] bg-white/95 backdrop-blur-xl border-t border-slate-100 p-5 px-6 md:px-12 shadow-[0_-15px_50px_rgba(0,0,0,0.1)] transition-all duration-500 transform ${showStickyBar ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="relative group">
               <img src={product.image} className="w-14 h-14 rounded-2xl object-cover shadow-lg group-hover:scale-105 transition-transform" />
            ...
            <div className="hidden sm:block">
              <h4 className="font-black text-slate-900 text-sm uppercase tracking-tight">{product.name}</h4>
              <p className="text-rose-800 font-black text-lg">{(product.price * quantity).toLocaleString()}৳</p>
            </div>
          </div>
          <div className="flex items-center gap-4 md:gap-8">
            <div className="flex items-center bg-slate-50 border border-slate-200 rounded-full p-1.5 shadow-sm">
            ...
            <button 
              onClick={() => onOrderNow(product, quantity)} 
              className="px-10 md:px-16 py-5 bg-rose-600 hover:bg-rose-700 text-white rounded-full font-black uppercase text-xs tracking-widest shadow-2xl flex items-center gap-3 transition-all active:scale-95"
            >
              <ShoppingBag className="w-5 h-5" /> Buy {(product.price * quantity).toLocaleString()}৳
            </button>
          </div>
        </div>
      </div>
