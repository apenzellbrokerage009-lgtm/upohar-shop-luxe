...
import { X, Truck, ShieldCheck, ShoppingBag, Phone, User as UserIcon, MapPin, RefreshCw, CheckCircle2 } from 'lucide-react';
...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, product, quantity, user, onOrder, onUpdateDraft }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onOrder({ ...formData, deliveryCharge: selectedZone.fee, ipAddress: trackingData.ip, location: trackingData.loc });
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white w-full max-w-xl h-full sm:h-auto sm:rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in slide-in-from-bottom duration-300 flex flex-col">
        ...
        <button onClick={onClose} className="absolute top-6 right-6 p-2 bg-slate-100 hover:bg-rose-100 hover:text-rose-800 rounded-full transition-all z-10"><X className="w-5 h-5" /></button>
        <div className="p-8 md:p-12 overflow-y-auto max-h-[90vh]">
          ...
          <div className="space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="p-6 bg-slate-900 rounded-[2.5rem] text-white"><div className="flex justify-between text-xs font-bold text-slate-400 uppercase tracking-widest mb-2"><span>Subtotal</span><span>{subtotal.toLocaleString()}৳</span></div><div className="flex justify-between text-xs font-bold text-slate-400 uppercase tracking-widest mb-4"><span>Delivery</span><span className="text-rose-400">+{selectedZone.fee}৳</span></div><div className="h-px bg-white/10 mb-4"></div><div className="flex justify-between items-end"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Payable</p><p className="text-3xl font-black text-rose-500">{total.toLocaleString()}৳</p></div></div>
