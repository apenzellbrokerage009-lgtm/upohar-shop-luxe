...
import { Plus, Edit3, Trash2, Check, X, Star, Sparkles, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setEditing({ category: state.categories[0]?.name, isFeatured: false, stock: 0 })} className="px-8 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase flex items-center gap-2 shadow-xl hover:bg-rose-600 transition-all"><Plus className="w-4 h-4"/> New Asset</button>
      </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-3xl rounded-[3.5rem] p-8 md:p-12 shadow-2xl max-h-[90vh] overflow-y-auto scrollbar-hide animate-in zoom-in duration-300">
             ...
             <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Collection / Category</label>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <input type="number" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-sm outline-none" placeholder="0.00" value={editing.price} onChange={e => setEditing({...editing, price: Number(e.target.value)})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Buying Price (৳)</label>
                    <input type="number" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-sm outline-none" placeholder="0.00" value={editing.buyingPrice} onChange={e => setEditing({...editing, buyingPrice: Number(e.target.value)})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Stock Level</label>
                    <input type="number" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-sm outline-none" placeholder="0" value={editing.stock} onChange={e => setEditing({...editing, stock: Number(e.target.value)})} />
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Short Description (Punchline)</label>
                  <input className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-sm outline-none" placeholder="A brief catchy sentence about the gift..." value={editing.shortDescription} onChange={e => setEditing({...editing, shortDescription: e.target.value})} />
                </div>

                <div className="space-y-2">
                  <div className="flex justify-between items-center ml-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Full Description (The Story)</label>
                    <button 
                      onClick={handleAiDescription}
                      disabled={isGenerating}
                      className="flex items-center gap-2 text-[10px] font-black text-rose-600 uppercase tracking-widest hover:text-rose-700 transition-colors disabled:opacity-50"
                    >
                      {isGenerating ? <Loader2 className="w-3 h-3 animate-spin"/> : <Sparkles className="w-3 h-3"/>}
                      Generate with AI
                    </button>
                  </div>
                  <textarea className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-3xl font-medium text-sm h-32 resize-none outline-none focus:ring-2 focus:ring-rose-600/20" placeholder="Describe the quality, materials, and emotion behind this gift..." value={editing.fullDescription} onChange={e => setEditing({...editing, fullDescription: e.target.value})} />
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Main Image URL</label>
                  <input className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-sm outline-none" placeholder="https://images.unsplash.com/..." value={editing.image} onChange={e => setEditing({...editing, image: e.target.value})} />
                </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-rose-200 transition-colors">
                  ...
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-slate-900">Feature on Homepage</span>
                    <span className="text-[9px] text-slate-400 uppercase font-black tracking-widest">Mark as premium collection item</span>
                  </div>
                </label>

                <button onClick={saveProduct} className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black uppercase text-xs tracking-[0.3em] hover:bg-rose-600 transition-all shadow-2xl flex items-center justify-center gap-3">
                  <Check className="w-5 h-5" /> Save to Catalog
                </button>
             </div>
          </div>
        </div>
